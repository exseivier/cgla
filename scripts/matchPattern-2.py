...
def main():
	...
	pattern = argv[2] # kmer to search for in sequences
	mismatch_allowed = int(argv[3]) # mismatch allowed
	output = pattern + "_" + str(mismatch_allowed) + "m.cat" # output file name
	...
	for header, items in matches.iteritems():
		...
		for position in positions:
			...
			elif position >= p4 and position <= lenseq - len(pattern):
