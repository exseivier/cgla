	...
	if (begin) {
		if (b.strand == "+"){
			if (b.start - chr.start > max_size) {
				...
				pos <- new("pocketOfSeq", chrname = b.chrname, name = b.name, seq = subseq(sequences[names(sequences) %in% b.chrname], 1, b.start))
				return(pos)
			}
		}
		# IF b.strand != "-"
	...
	# IF BEGIN = FALSE
		...
		# IF b.strand != "-"
			...
			if (c.start - b.end > max_size | c.start - b.end < 0) {
				...
				pos <- new("pocketOfSeq", chrname = b.chrname, name = b.name, seq = reverseComplement(subseq(sequences[names(sequences) %in% b.chrname], b.edn, c.start)))
				return(pos)
			}
		}
	}
